...
It is non-zero only for those partonic subprocesses which involve identical quarks or anti-quarks.
...
\section{Details of the calculations}
    ...
    The initial-state gluon and quark 
        must not couple together, otherwise infrared divergences proportional to $s$-channels appear, 
    ...
    In the following, the codes employed throughout this paper and the approximations implemented in each of them are discussed:
    ...
    The virtual corrections are additionally approximated using a double-pole approximation where the final state leptons are assumed to originate from the decay of two resonant W-bosons.\MP{Specifiy what type of DPA by citation}
...
output
    ...
    It relies on the user providing the matrix elements and Born phase space, but automatically constructs FKS \cite{Frixione:1995ms} subtraction terms and the phase space for the real emission.
    ...
    It provides FKS subtraction terms for any NLO process, while virtual matrix
...
\section{Leading-order study}
    ...
    The full calculation is approximated at the level of $\pm5\%$ apart from the region where $|\Delta y_{\Pj\Pj}| < 2$.
    ...
    At the level of the cross section, it seems difficult to infer the quality of the various approximations.
    ...
    This is in agreement with the findings of Refs.~\cite{Denner:2012dz,Oleari:2003tc}.
    ...
    The upper plots provides the absolute value for each prediction while the lower plots presents all predictions normalised to {\sc MoCaNLO}+{\sc Recola} which is one of the programs that provides the full prediction.
...
\section{Next-to-leading order QCD}
    ...
    The inclusion of $s$-channels cures partly this behaviour by improving the approximation to about $10\%$.
    ...
    The upper plots provide the absolute value for each prediction while the lower plots present all predictions normalised to {\sc MoCaNLO}+{\sc Recola} which is one of the programs that provides the full prediction.
    In addition to the cuts of Sec.~\protect\ref{subsec:inputpar}, the VBS cuts take the values $m_{\Pj\Pj}>200 \GeV$ and $|\Delta y_{\Pj\Pj}|>2$.} 
    ...
    \caption{Ratios for double-differential distributions in the variables $m_{\Pj\Pj}$ and $|\Delta y_{\Pj\Pj}|$ at NLO QCD \emph{i.e.}\ at order $\mathcal{O}(\alphas\alpha^6)$ for the VBS approximation over the full computation of the approximated squared amplitudes over the full matrix element.
    The approximated squared amplitudes are computed as $|\mathcal{A}|^2 \sim |t|^2+|u|^2$ (left) and $|\mathcal{A}|^2 \sim |s|^2+|t|^2+|u|^2$ (right).
    In addition to the cuts of Sec.~\protect\ref{subsec:inputpar}, the VBS cuts take the values $m_{\Pj\Pj}>200 \GeV$ and $|\Delta y_{\Pj\Pj}|>2$.}
    ...
    \end{equation}
    %
    Analogous definitions are later also be used for the Zeppenfeld variable of the muon and of the third jet.
    %
    \begin{figure*}
    \centering
    ...
    The upper plots provide the absolute value for each prediction while the lower plots presents all predictions normalised to {\sc MoCaNLO}+{\sc Recola} which is one of the programs that provides the full prediction.
    In addition to the cuts of Sec.~\protect\ref{subsec:inputpar}, the VBS cuts take the values $m_{\Pj\Pj}>200 \GeV$ and $|\Delta y_{\Pj\Pj}|>2$.} 
    ...
    In the inclusive region studied here, neglecting $s$-channel, non-factorisable corrections, and EW corrections can lead to discrepancies of up to $30\%$ with respect to the full computation.
    ...
    As real radiation, an extra gluon-jet can be radiated from any of the charged particles while the two quarks originating from the W-boson decay can be recombined in a single jet.
...
well as the dipole shower can be employed. We here use the subtractive (MC@NLO-type) matching to these parton-show\-er algorithms.
...
For this observable (and all the others which are NLO accurate) the effect of reducing the shower-scale is negligible, hence the corresponding histogram is not be shown.
...
clearly suggest not to rely on a single tool/parton shower, and on the other make it worth to investigate more in detail the way QCD radiation is
