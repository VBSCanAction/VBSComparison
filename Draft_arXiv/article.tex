...
Nonetheless they share the same final state, and therefore constitutes an irreducible background to EW process.
...
\section{Details of the calculations}
    ...
    The virtual corrections are additionally approximated using a double-pole approximation where the final state leptons are assumed to originate from the decay of two resonant W-bosons.\MP{Specify what type of DPA by citation}
...
output
    ...
    It relies on the user providing the matrix elements and Born phase space, but automatically constructs FKS \cite{Frixione:1995ms} subtraction terms and the phase space phase corresponding to the real emission.
    ...
    the VBS channels in terms of both higher-dimensional operators as well as explicit
    resonances.
    ...
    \begin{table*}
        ...
        \begin{tabularx}{\textwidth}{c|c|X|X|X|X|X}
            ...
            \hline
            \hline
            {\sc Bonsay}        &  $t,u$    &  No       &  No		    &  Yes   & No       &  No  \\
            {\sc Powheg}        &  $t,u$    &  No       &  No               &  Yes   & No       &  No  \\
    ...
    \end{equation}
    %
    and where $M_V$ corresponds to the real part of the squared pole mass.
...
\section{Leading-order study}
    ...
    integrators performing too aggressive estimations of statistical uncertainties or different implementation of the complex-mss scheme.
    ...
    for which we also display the scale-uncertainty band (seven-points variation defined as in Eq.~(3.11) of Ref.~\cite{Biedermann:2017bss}).
...
among tools are reduced to about $10\%$. For the three-jet bin, which is described only at LO accuracy\MP{Here only MG has the full ME and we have argued that the s-channel contribution can be large in the real. A comment might be in order.}, differences among tools remain large: the largest rate is predicted by
...
For this observable (and all the others which are NLO accurate) the effect of reducing the shower-scale is negligible, hence the corresponding histogram is not shown.
