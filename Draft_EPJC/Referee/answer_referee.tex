...
the matrix-elements in a phase-space point, finding very good agreement. Other possible sources of the differences 
may originate from the factorization scale setting or the cuts, or, more likely, from the non-perfect or non-ideal
coverage of the VBS phase space, which is rather complex (in particular when also non-resonant contributions have to
be included). Another issue of concern, as we alredy have written in the submitted version, is whether one should
...
\begin{quote}
    ...
    of the various VBS approximations have an impact well below 1\% at the level of the fiducial cross section at LO for a
