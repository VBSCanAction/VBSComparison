...
%DIF DEL ../Draft_arXiv/article.tex   Mon Jun 11 10:17:24 2018
%DIF ADD article.tex                  Fri Jun 15 10:09:07 2018
...
evidence for it was found at the centre-of-mass energy of $8~\TeV$\DIFdelbegin \DIFdel{already~\mbox{%DIFAUXCMD
\cite{Aad:2014zda,Khachatryan:2014sta}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\cite{Aad:2014zda,Khachatryan:2014sta,Aaboud:2016ffv}}%DIFAUXCMD
}\DIFaddend ,
and it has been recently \DIFdelbegin \DIFdel{observed~\mbox{%DIFAUXCMD
\cite{Sirunyan:2017ret} }%DIFAUXCMD
and measured
~\mbox{%DIFAUXCMD
\cite{Aaboud:2016ffv} 
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{measured
}\DIFaddend at $13\TeV$ as well\DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\cite{Sirunyan:2017ret}}%DIFAUXCMD
}\DIFaddend .
...
The $s$-channel contributions plays a particular role in the study of the various contributions in Sec.~\ref{subsec:contributions}.
...
In \DIFaddbegin \DIFadd{the rest of this article, the notations }\emph{\DIFadd{LO}} \DIFadd{or }\emph{\DIFadd{NLO(-QCD)}} \DIFadd{without any specification of coupling powers refer to the contributions at order $\mathcal{O}{\left(\alpha^{6}\right)}$ and $\mathcal{O}{\left(\alpha_{\rm s}\alpha^{6}\right)}$, respectively.
}

\DIFadd{In }\DIFaddend experimental measurements, special cuts, called VBS cuts, are designed to enhance the EW contribution over the QCD one and to suppress the interference.
...
defined.
Hence, combined measurements which are \DIFdelbegin \DIFdel{better theoretically }\DIFdelend \DIFaddbegin \DIFadd{theoretically better }\DIFaddend defined should be preferably performed by the experimental collaborations at the LHC.
...
\section{Details of the calculations}
    ...
    In the following, the codes employed throughout this article and the approximations implemented in each of them are discussed:
     ...
     \begin{itemize} 
      \DIFaddbegin \item {\sc \DIFadd{Phantom}}\DIFadd{~\mbox{%DIFAUXCMD
\cite{Ballestrero:2007xq} }%DIFAUXCMD
is a dedicated tree-level Monte Carlo for six-parton final states 
    ...
    It employs complete tree-level matrix elements in the complex-mass scheme~\mbox{%DIFAUXCMD
\cite{Denner:1999gp,Denner:2005fg,Denner:2006ic} }%DIFAUXCMD
computed via the modular helicity formalism~\mbox{%DIFAUXCMD
\cite{Ballestrero:1999md,Ballestrero:1994jn}}%DIFAUXCMD
.
    The integration uses a multi-channel approach~\mbox{%DIFAUXCMD
\cite{Berends:1984gf} }%DIFAUXCMD
and an adaptive strategy~\mbox{%DIFAUXCMD
\cite{Lepage:1977sw}}%DIFAUXCMD
.
    }{\sc \DIFadd{Phantom}} \DIFadd{generates unweighted events at parton level for both the SM and a few instances of beyond the Standard Model (BSM) theories.
}\DIFaddend 

      \item \DIFaddbegin \DIFadd{Whizard~\mbox{%DIFAUXCMD
\cite{Kilian:2007gr,Moretti:2001zz} }%DIFAUXCMD
is a multi-purpose event generator with LO matrix-element generator O'Mega. 
      For QCD amplitudes it uses the colour flow formalism~\mbox{%DIFAUXCMD
\cite{Kilian:2012pz}}%DIFAUXCMD
.
      For NLO QCD calculations~\mbox{%DIFAUXCMD
\cite{Nejad:2016bci}}%DIFAUXCMD
, where Whizard is in the final validation phase, it provides FKS subtraction terms~\mbox{%DIFAUXCMD
\cite{Frixione:1995ms,Frixione:1997np}}%DIFAUXCMD
, while virtual matrix elements are provided externally by OpenLoops~\mbox{%DIFAUXCMD
\cite{Cascioli:2011va} }%DIFAUXCMD
or Recola~\mbox{%DIFAUXCMD
\cite{Actis:2012qn,Actis:2016mpe}}%DIFAUXCMD
.
      Furthermore, Whizard can automatically provide POWHEG matching to parton shower~\mbox{%DIFAUXCMD
\cite{Reuter:2016qbi}}%DIFAUXCMD
.
      Whizard allows to simulate a huge number of BSM models as well, in particular in terms of higher-dimensional operators for VBS processes including means to provide unitarity limits~\mbox{%DIFAUXCMD
\cite{Alboteanu:2008my,Kilian:2014zja}}%DIFAUXCMD
.
      }

    \item \DIFaddend The program {\sc Bonsay}~\cite{Dittmaier:2018zzz} consists of a general-purpose Monte Carlo integrator and matrix elements taken from different sources:
...
detailed description and Ref.~\cite{Dittmaier:2015bfe} for the on-shell projection used.
    At LO the exact matrix elements can also be used.
      ...
      \item \DIFaddbegin \DIFadd{The }\DIFaddend {\sc \DIFaddbegin \DIFadd{Powheg-Box}}\DIFadd{~\mbox{%DIFAUXCMD
\cite{Nason:2004rx,Frixione:2007vw,Alioli:2010xd} }%DIFAUXCMD
is a framework for matching NLO-QCD calculations with parton showers.
    It relies on the user providing the matrix elements and Born phase space, but automatically constructs FKS \mbox{%DIFAUXCMD
\cite{Frixione:1995ms} }%DIFAUXCMD
subtraction terms and the phase space corresponding to the real emission.
    For the VBS processes all matrix elements are being provided by a previous version of }{\sc \DIFadd{VBFNLO}}\DIFadd{~\mbox{%DIFAUXCMD
\cite{Arnold:2008rz, Arnold:2011wj, Baglio:2014uba} }%DIFAUXCMD
and hence the approximations used in the }{\sc \DIFadd{Powheg-Box}} \DIFadd{are similar to those used in }{\sc \DIFadd{VBFNLO}}\DIFadd{.
}

      \item {\sc \DIFadd{VBFNLO}}\DIFadd{~\mbox{%DIFAUXCMD
\cite{Arnold:2008rz, Arnold:2011wj, Baglio:2014uba} }%DIFAUXCMD
is a flexible
        ...
        Ref.~\mbox{%DIFAUXCMD
\cite{Jager:2006zc}}%DIFAUXCMD
, thereby leads to a significant speed improvement over
      ...
      \item {\sc \DIFaddend MadGraph5\_aMC@NLO}~\cite{Alwall:2014hca} (henceforth {\sc MG5\-\_aMC}) is an automatic meta-code (a code that generates codes) which makes it possible to simulate any scattering process
...
output
    ...
    \DIFdelbegin %DIFDELCMD < \item {\sc %%%
\DIFdel{Phantom}%DIFDELCMD < }%%%
...
\DIFdel{Phantom}%DIFDELCMD < } %%%
\DIFdel{generates unweighted events at parton level for both the SM and a few instances of beyond the Standard Model (BSM) theories.
}%DIFDELCMD < 

%DIFDELCMD <       \item %%%
\item%DIFAUXCMD
\DIFdel{The }%DIFDELCMD < {\sc %%%
\DIFdel{Powheg-Box}%DIFDELCMD < }%%%
\DIFdel{~\mbox{%DIFAUXCMD
\cite{Nason:2004rx,Frixione:2007vw,Alioli:2010xd} }%DIFAUXCMD
is a framework for matching NLO-QCD calculations with parton showers.
    It relies on the user providing the matrix elements and Born phase space, but automatically constructs FKS \mbox{%DIFAUXCMD
\cite{Frixione:1995ms} }%DIFAUXCMD
subtraction terms and the phase space corresponding to the real emission.
    For the VBS processes all matrix elements are being provided by a previous version of }%DIFDELCMD < {\sc %%%
\DIFdel{VBFNLO}%DIFDELCMD < }%%%
\DIFdel{~\mbox{%DIFAUXCMD
\cite{Arnold:2008rz, Arnold:2011wj, Baglio:2014uba} }%DIFAUXCMD
and hence the approximations used in the }%DIFDELCMD < {\sc %%%
\DIFdel{Powheg-Box}%DIFDELCMD < } %%%
\DIFdel{are similar to those used in }%DIFDELCMD < {\sc %%%
\DIFdel{VBFNLO}%DIFDELCMD < }%%%
\DIFdel{.
}%DIFDELCMD < 
...
\DIFdelend  \end{itemize} 
    ...
    \begin{table*}
        ...
        \begin{tabularx}{\textwidth}{c|c|X|X|X|X|X}
            ...
            \hline
            \hline
            {\sc \DIFdelbeginFL \DIFdelFL{Bonsay}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Phantom}\DIFaddendFL }       &  \DIFdelbeginFL \DIFdelFL{$t,u$    }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$s,t,u$  }\DIFaddendFL &  \DIFdelbeginFL \DIFdelFL{No       }%DIFDELCMD < &  %%%
\DIFdelFL{Yes      , virt. No    }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Yes      }\DIFaddendFL &  Yes              &  No    & \DIFdelbeginFL \DIFdelFL{No  }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{-        }& \DIFaddFL{- }\DIFaddendFL \\
            {\sc \DIFdelbeginFL \DIFdelFL{Powheg}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Whizard}\DIFaddendFL }       &  \DIFdelbeginFL \DIFdelFL{$t,u$    }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$s,t,u$  }\DIFaddendFL &  \DIFdelbeginFL \DIFdelFL{No       }%DIFDELCMD < &  %%%
\DIFdelendFL Yes      &  Yes              &  No    & \DIFdelbeginFL \DIFdelFL{No  }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{-        }& \DIFaddFL{- }\DIFaddendFL \\
            {\sc \DIFdelbeginFL \DIFdelFL{MG5\_aMC}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Bonsay}\DIFaddendFL }        &  \DIFdelbeginFL \DIFdelFL{$s,t,u$  }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$t,u$    }\DIFaddendFL &  \DIFdelbeginFL \DIFdelFL{Yes      }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{No       }\DIFaddendFL &  Yes\DIFaddbeginFL \DIFaddFL{, virt. No    }\DIFaddendFL &  Yes   & \DIFdelbeginFL \DIFdelFL{virt. }\DIFdelendFL No       &  No  \\
            {\sc \DIFdelbeginFL \DIFdelFL{MoCaNLO+Recola}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Powheg}\DIFaddendFL }        &  \DIFdelbeginFL \DIFdelFL{$s,t,u$  }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$t,u$    }\DIFaddendFL &  \DIFdelbeginFL \DIFdelFL{Yes      }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{No       }\DIFaddendFL &  Yes              &  Yes   & \DIFdelbeginFL \DIFdelFL{Yes      }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{No       }\DIFaddendFL &  \DIFdelbeginFL \DIFdelFL{Yes  }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{No  }\DIFaddendFL \\
            {\sc \DIFdelbeginFL \DIFdelFL{PHANTOM}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{VBFNLO}\DIFaddendFL }        &  $s,t,u$  &  \DIFaddbeginFL \DIFaddFL{No       }&  \DIFaddendFL Yes              &  Yes   & No       &  \DIFdelbeginFL \DIFdelFL{-        }%DIFDELCMD < & %%%
\DIFdelFL{- }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{No  }\DIFaddendFL \\
            {\sc \DIFdelbeginFL \DIFdelFL{VBFNLO}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{MG5\_aMC}\DIFaddendFL }      &  $s,t,u$  &  \DIFdelbeginFL \DIFdelFL{No       }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Yes      }\DIFaddendFL &  Yes              &  Yes   & \DIFaddbeginFL \DIFaddFL{virt. }\DIFaddendFL No &  No \\
            {\sc \DIFdelbeginFL \DIFdelFL{Whizard}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{MoCaNLO+Recola}\DIFaddendFL }&  $s,t,u$  &  Yes      &  Yes              &  \DIFdelbeginFL \DIFdelFL{No    }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Yes   }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{-        }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Yes      }\DIFaddendFL &  \DIFdelbeginFL \DIFdelFL{- }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Yes  }\DIFaddendFL \\
        \end{tabularx}
...
\section{Leading-order study}
    ...
    The full calculation is approximated at the level of $\pm5\%$ everywhere apart in the region where $|\Delta y_{\Pj\Pj}| < 2$.
    ...
    described in Sec.~\ref{subsec:inputpar}. \DIFdelbegin \DIFdel{We note that several full predictions are not in statistical agreement. These are possibly due to Monte Carlo 
    integrators performing too aggressive estimations of statistical uncertainties or different implementation of the complex-mass scheme.
    Nonetheless, all these predictions agree within less than $0.5\%$.
    At the level of the cross section, 
    it seems difficult to infer the quality of the various approximations}\DIFdelend \DIFaddbegin \DIFadd{Two things should be highlighted here: first, despite the different underlying approximations, 
    the two most-distant predictions (}{\sc \DIFadd{Powheg-box}}\DIFadd{\ and }{\sc \DIFadd{MG5\_aMC}}\DIFadd{) are only $0.7\%$ apart}\DIFaddend . This simply means that the details 
    of the various VBS approximations have an impact \DIFdelbegin \DIFdel{not larger than $0.5\%$ }\DIFdelend \DIFaddbegin \DIFadd{below $1\%$ }\DIFaddend at the level of the fiducial cross section at LO for a 
    typical phase-space volume used by experimental collaborations. This is in agreement with the findings of 
    Refs.~\cite{Denner:2012dz,Oleari:2003tc}. \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD <     %%%
\DIFdelend \DIFaddbegin \DIFadd{Second, the four complete predictions (}{\sc \DIFadd{Whizard}}\DIFadd{, }{\sc \DIFadd{Phantom}}\DIFadd{, }{\sc \DIFadd{MG5\_aMC}}\DIFadd{, 
    and }{\sc \DIFadd{MoCaNLO+Recola}}\DIFadd{) are not in statistical agreement. While we have checked the point-wise agreement of the matrix-element, 
    we cannot exclude other reasons for the disagreement, for example a non representative (}\emph{\DIFadd{i.e.}}\DIFadd{\ too-aggressive) 
    ...
    we deem satisfactory compared to the larger differences observed at NLO or when including matching to parton shower. 
        ...
        \begin{tabular}{c|r@{ $\pm$ }l}
          Code  &  \multicolumn{2}{c}{$\sigma[\rm{fb}]$}  \\
            ...
            {\sc \DIFdelbeginFL \DIFdelFL{MG5\_aMC}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Powheg-Box}\DIFaddendFL }  &  \DIFdelbeginFL \DIFdelFL{$1.4304\phantom{0}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$1.44092$ }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{$0.0007$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$0.00009$ }\DIFaddendFL \\
            {\sc \DIFdelbeginFL \DIFdelFL{MoCaNLO+Recola}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{VBFNLO}\DIFaddendFL }  &  \DIFdelbeginFL \DIFdelFL{$1.43476$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$1.43796$ }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{$0.00009$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$0.00005$ }\DIFaddendFL \\
            {\sc \DIFdelbeginFL \DIFdelFL{PHANTOM}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Phantom}\DIFaddendFL } &  $1.4374\phantom{0}$ & $0.0006 $  \\
            {\sc \DIFdelbeginFL \DIFdelFL{Powheg-Box}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Whizard}\DIFaddendFL } &  \DIFdelbeginFL \DIFdelFL{$1.44092$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$1.4381\phantom{0}$ }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{$0.00009$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$0.0002 $ }\DIFaddendFL \\
            {\sc \DIFdelbeginFL \DIFdelFL{VBFNLO}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{MG5\_aMC}\DIFaddendFL }&  \DIFdelbeginFL \DIFdelFL{$1.43796$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$1.4304\phantom{0}$ }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{$0.00005$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$0.0007$ }\DIFaddendFL \\
            {\sc \DIFdelbeginFL \DIFdelFL{Whizard}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{MoCaNLO+Recola}\DIFaddendFL }  &  \DIFdelbeginFL \DIFdelFL{$1.4381\phantom{0}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$1.43476$ }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{$0.0002 $
        }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$0.00009$ 
        }\DIFaddendFL \end{tabular}
        \caption{\label{tab:wg1_LOrates} Cross sections at LO accuracy and order $\mathcal{O}(\alpha^6)$.
        The \DIFaddbeginFL \DIFaddFL{complete $2\to 6$ matrix-element, without any approximation, is employed by }{\sc \DIFaddFL{Phantom}}\DIFaddFL{,
        }{\sc \DIFaddFL{Whizard}}\DIFaddFL{, }{\sc \DIFaddFL{MG5\_aMC}} \DIFaddFL{and }{\sc \DIFaddFL{MoCaNLO+Recola}}\DIFaddFL{. The }\DIFaddendFL predictions are obtained 
    ...
    \begin{table}
        ...
        \begin{tabular}{c|r@{ $\pm$ }l}
          Code  &  \multicolumn{2}{c}{$\sigma[\rm{fb}]$}  \\
            ...
            {\sc \DIFaddbeginFL \DIFaddFL{VBFNLO}}  &  \DIFaddFL{$1.3916\phantom{0}$ }& \DIFaddFL{$0.0001$  }\\
            {\sc \DIFaddendFL MG5\_aMC}&  $1.363\phantom{0}\phantom{0}$ & $0.004$  \\
            {\sc MoCaNLO+Recola}  &  $ 1.378\phantom{0}\phantom{0}$ & $0.001$ 
        \DIFdelbeginFL %DIFDELCMD < \\
%DIFDELCMD <             {\sc %%%
\DIFdelFL{VBFNLO}%DIFDELCMD < }  &  %%%
\DIFdelFL{$1.3916\phantom{0}$ }%DIFDELCMD < & %%%
...
\DIFdelendFL \end{tabular}
    ...
    \caption{\label{fig:distNLO3} Differential distributions in the invariant mass of the two charged leptons (left) and Zeppenfeld variable for the muon (right) \emph{i.e.}\ at NLO accuracy at order $\mathcal{O}(\alpha_{\rm s}\alpha^6)$.
    The description of the different programs used can be found in Sec.~\protect\ref{subsec:codedescr}.
    The upper plots provide the absolute value for each prediction while the lower plots present all predictions normalised to {\sc MoCaNLO}+{\sc Recola} which is the full prediction.
    The band corresponds to a seven-point variation of the renormalisation and factorisation scales.
    The predictions are obtained in the fiducial region described in Sec.~\protect\ref{subsec:inputpar}.
...
\section{Matching to parton shower}
    \label{sec:matching}
We now discuss how different predictions compare when the matching to parton shower is included\DIFaddbegin \DIFadd{, both at LO 
(}\emph{\DIFadd{i.e.}}\DIFadd{\ at order $\mathcal O (\alpha^6)$) and at NLO-QCD (}\emph{\DIFadd{i.e.}}\DIFadd{\ at order $\mathcal O (\alpha^6\alphas)$) accuracy}\DIFaddend . For such
...
also {\sc \DIFdelbegin \DIFdel{PHANTOM}\DIFdelend \DIFaddbegin \DIFadd{Phantom}\DIFaddend } and {\sc \DIFdelbegin \DIFdel{WHIZARD}\DIFdelend \DIFaddbegin \DIFadd{Whizard}\DIFaddend } are used for LO+PS results.\\
...
The same parton showers are employed for the LO results of {\sc \DIFdelbegin \DIFdel{PHANTOM}\DIFdelend \DIFaddbegin \DIFadd{Phantom}\DIFaddend }.
{\sc Pythia8} is also employed for the LO results of {\sc \DIFdelbegin \DIFdel{WHIZARD}\DIFdelend \DIFaddbegin \DIFadd{Whizard}\DIFaddend }. For the {\sc Powheg-Box}, the namesake
...
\begin{table}
    ...
    \begin{tabular}{c|l@{ $\pm$ }l}
      Code  &  \multicolumn{2}{c}{$\sigma[\rm{fb}]$}  \\
        ...
        {\sc \DIFdelbeginFL \DIFdelFL{PHANTOM}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Phantom}\DIFaddendFL }+{\sc Pythia8} &  $1.235 $ & $0.001$  \\
        {\sc \DIFdelbeginFL \DIFdelFL{PHANTOM}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Phantom}\DIFaddendFL }+{\sc Herwig7} &  $1.258 $ & $0.001$  \\
        {\sc VBFNLO}+{\sc Herwig7-Dipole} &  $1.3001$ & $0.0002$  \\
        {\sc \DIFdelbeginFL \DIFdelFL{WHIZARD}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Whizard}\DIFaddendFL }+{\sc Pythia8} &  $1.229$ & $0.001$  \\
    \end{tabular}
...
possibly related to the underlying approximation performed ({\sc MG5\_aMC} is the only tool to use the full matrix \DIFdelbegin \DIFdel{elment }\DIFdelend \DIFaddbegin \DIFadd{element }\DIFaddend for the real radiation), in particular the inclusion of the $s$-channel contributions: the largest rate is predicted by
...
Some exceptions exist, \emph{e.g.}\ the rapidity separation of the two tagging jets, which \DIFaddbegin \DIFadd{on the one hand
}\DIFaddend clearly suggest not to rely on a single tool/parton shower, and on the other make it worth investigating more in detail the way QCD radiation is
...
underestimates the size of the perturbative uncertainty. Again, this stresses the need
 to employ at least two different tools in order to obtain a \DIFdelbegin \DIFdel{conservative }\DIFdelend \DIFaddbegin \DIFadd{more realistic }\DIFaddend estimate of theoretical uncertainties. Finally, the size of discrepancies for observables that are described at a lower perturbative accuracy, notably those related to the third jet, suggests that
