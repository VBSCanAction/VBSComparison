...
In the rest of this paper, the notation ``LO'' or ``NLO(-QCD)'' without any specification of coupling powers will refer to the contribution at $\mathcal{O}{\left(\alpha^{6}\right)}$ and $\mathcal{O}{\left(\alpha_{\rm s}\alpha^{6}\right)}$ respectively.  
...
($\mathcal O (\alpha^6)$) and at NLO-QCD ($\mathcal O (\alpha^6\alphas)$) accuracy. For such
