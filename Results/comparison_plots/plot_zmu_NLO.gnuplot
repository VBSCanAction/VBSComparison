...
plot \
"<paste ../VBFNLO/NLO/hist.zmu.dat ../VBFNLO/NLO/hist.zmu.dat" u (($1+$2)/2):($15*VBFNLO_fact*($2-$1)/($7*VBFNLO_fact*($2-$1))) ls 1 t 'VBFNLO',\
"<paste ../VBFNLO/NLO/hist.zmu.dat ../MG5_aMC/NLO/z_mu_NLO.dat" u (($1+$2)/2):($11)/($7*VBFNLO_fact*($2-$1)) ls 2 t 'MG5\_aMC',\
"<paste ../VBFNLO/NLO/hist.zmu.dat ../POWHEG/NLO/zstar_-muon-_VBF_CUTS_index__13.dat" u (($1+$2)/2):($11*POWHEG_fact*($2-$1)/($7*VBFNLO_fact*($2-$1))) ls 3 t 'POWHEG',\
"<paste ../VBFNLO/NLO/hist.zmu.dat ../Recola/NLO/histogram_zeppenfeld_zmup_nlo.dat" u (($1+$2)/2):($12*RECOLA_fact*($2-$1)/($7*VBFNLO_fact*($2-$1))) ls 4 t 'Recola',\
"<paste ../VBFNLO/NLO/hist.zmu.dat ../BONSAY/NLO/ew-nlo.zmu" u (($1+$2)/2):(BONSAY_fact*$10*0.05/($7*VBFNLO_fact*($2-$1))) ls 6 t 'BONSAY',\
