...
%DIF DEL article_before_qsh.tex   Tue Mar 13 09:49:51 2018
%DIF ADD article.tex              Tue Mar 13 11:37:38 2018
...
\begin{abstract}
  ...
  \DIFaddbegin \DIFadd{MCnet-18-06, }\DIFaddend Nikhef/2018-012, UWTHPH-2018-12, 
  \DIFaddbegin 

  \DIFaddend VBSCan-PUB-01-18, \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD <   %%%
\DIFdelend ZU-TH-08/18}
\end{abstract}
\thispagestyle{empty}
\vfill
\newpage
...
\section{Details of the calculations}
    ...
    For the VBS processes all matrix elements are being provided by a previous version of {\sc VBFNLO}~\cite{Arnold:2008rz, Arnold:2011wj, Baglio:2014uba} and hence the approximations used in the {\sc Powheg-Box} are similar to those used in {\sc VBFNLO}.
      ...
      \item {\sc VBFNLO}~\cite{Arnold:2008rz, Arnold:2011wj, Baglio:2014uba} is a flexible
        ...
        approximation, with process IDs between 200 and 290. \DIFaddbegin \DIFadd{Same-sign
        $\PW^+\PW^+jj$ production is provided via the process ID 250. }\DIFaddend The corresponding
        $s$-channel contributions are available separately as tri-boson processes with
        semi-leptonic decays, with process IDs \DIFdelbegin \DIFdel{in the
        400 range}%DIFDELCMD < \ZM{which range is it?}%%%
\DIFdelend \DIFaddbegin \DIFadd{between 401 and 492. For the
        final state studied in this article, only $\PW^+\PW^+\PW^-$
        production with a hadronically decaying $\PW^-$ boson, process ID 432,
        can contribute}\DIFaddend . These can simply be added on top of the VBS
...
interplay between the real-emission matrix element and the shower emission. \DIFaddbegin \DIFadd{For this observable we also show the prediction obtained
with }{\sc \DIFadd{MG5\_aMC+Pythia8}} \DIFadd{by reducing the shower-starting scale by a factor 2 with respect to the default value, dubbed 
}{\sc \DIFadd{MG5\_aMC+Py8, $Q_{sh}/2$}}\DIFadd{.}\footnote{\DIFadd{The reduction of the shower scale for }{\sc \DIFadd{MG5\_aMC+Herwig++}} \DIFadd{gives no visible effect on any of the observables 
discussed in this work.}} \DIFadd{The main effect of reducing the shower scale is that events migrate from the three-jet bin into the two-jet bin, i.e. less radiation is generated. The
...
about -30\% at the end of the displayed range. At NLO+PS, this effect is mitigated, owing to the better description of the first QCD emission which is now driven by the real-emission matrix element. \DIFaddbegin \DIFadd{For this observable (and all the others which are NLO accurate) the effect of reducing the shower-scale is 
...
the effect survives beyond the first emission, as it can be observed by comparing {\sc Powheg-LHE} with {\sc Powheg+ Pythia8}\DIFaddbegin \DIFadd{, and that
it is only marginally attenuated when the shower scale is reduced}\DIFaddend . A similar
