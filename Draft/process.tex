...
should perform an on-shell projection of the incoming and outgoing $\PW$ bosons \MZ{it is incoming and outgoing, right? in case modify what follows}.
...
All other codes ({\sc MG5\_aMC}, {\sc MoCaNLO+Recola}, {\sc PHANTOM}, and {\sc Whizard})\AK{CITATIONS} consider all contributions of order $\mathcal{O}{\left(\alpha^{6}\right)}$ as well as all possible interferences.
...
The full computation of order $\mathcal{O}{\left(\alphas \alpha^{6}\right)}$ consists thus not only of QCD-type corrections but also of EW ones.\footnote{For more details, the reader is referred to Ref.~\cite{Biedermann:2017bss}.\AK{Do we need this footnote given that the next sentence gives the reference?}} 
