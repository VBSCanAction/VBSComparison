...
\begin{itemize}
    ...
    \item The program {\sc Bonsay} consists of a general-purpose Monte Carlo integrator. It implements the VBS approximation at LO and NLO, with
        the exception of the virtual matrix elements, which are computed in the EVBA. Matrix elements are taken from several sources.
...
\end{verbatim}
  With these commands the complex-mass scheme is turned on {\tt \#1}, then the NLO-capable model is loaded {\tt \#2}\footnote{Despite
        ...
        and written to disk {\tt \#4}. No approximation is performed for the Born and real-emission matrix elements. 
        ...
        {\bf MZ what exactly needs to be clarified here? perhaps I am just too used to this that I do not see problems here}
...
In {\sc MoCaNLO+Recola} no approximation is performed at LO as well as at NLO, and, in particular, also contributions stemming from EW corrections to the interference are computed.
...
{\bf MZ : I would move these sentences in the setup part, possibly as footnotes}
The {\sc Powheg-Box} uses its own implementation of the two loop running for $\alpha_{\rm s}$.
The renormalisation and factorisation scale used differ slightly from the one defined in Eq.~\eqref{eq:defscale}, as rather than constructing the jets the {\sc Powheg-Box} uses the transverse momentum of the two final-state quarks in the underlying Born event. 
  ...
  \item {\sc Whizard}~\cite{Moretti:2001zz,Kilian:2007gr} is a multi-purpose
      event generator with the LO matrix element generator {\sc O'Mega}. {\bf MZ: if NLO results for this processes cannot be provided, we should skip what follows, or at least clarify the limitations}
...
We conclude this section by summarizing the details of the various codes in Tab.~\ref{tab:wg1_codes}. In particular, it is specified whether
\begin{itemize}
