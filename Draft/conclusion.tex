...
mainly focused on the electroweak production mechanism which involves the scattering fo massive vector bosons.
Until very recently, when the complete calculation became available for the NLO QCD corrections ($\mathcal O (\alphas\alpha^6)$),
...
which give the possibility to perform matching to parton showers have been employed, and various observables have been thoroughly compared.
...
These differences appear in the central region where for VBS, and are the largest for those simulations which employ {\sc Pythia8}. The effect
has been understood, and it can be somehow mitigated by changed the recoil scheme of the parton shower. These findings makes it worth 
...
\begin{itemize}
    \item As found in Ref.~\cite{Biedermann:2017bss}, the NLO EW corrections of order $\mathcal{O}{\left(\alpha^{7}\right)}$ are 
        ...
        It is thus highly desirable that they are combined with NLO-QCD predictions matched with parton shower, or at least that they accounted for
        by experimental analyses. Since, as shown in~\cite{Biedermann:2017bss}, 
        ...
        in a first approximation, by considering $\pm \delta^2_{\rm NLO EW}$, while the missing higher-order mixed QCD-EW corrections 
        can be estimated by taking the difference between the multiplicative and additive prescription.
        For more detailed studies of the combination of QCD and EW higher-order corrections, see
        e.g., Ref.~\cite{Czakon:2017wor} in the context of top-pair production, or Ref.~\cite{Lindert:2017olm} for SM 
...
%Therefore, jet-veto procedures or selections based on extra radiation should be avoided as they carry rather large uncertainties.
    ...
    \item In addition to the standard interpretation of EW signal versus QCD background, 
        combined measurements should also be presented as they are better defined theoretically. In fact, while at at LO 
        the interference term can be included in the background component, at NLO the separation of EW and QCD component become more blurred, as, e.g.,
        as at the order $\mathcal{O}{\left(\alphas\alpha^{6}\right)}$ both types of amplitudes mix.
        Therefore, a combined measurement including the EW, QCD, and interference contributions is desirable.
        \ZM{the following has to be clarified. one way would be to suggest to use  phase-space  cuts to enhance/suppress configurations 
            where EW/QCD are the dominant contributions}
...
Therefore we should probably discuss it all together to ensure that we all agree on a definite statement.}
    ...
    \item Since the inclusion of NLO QCD corrections gives a better control of extra QCD radiations and reduces the ambiguites related to the 
    ...
    (e.g. validity of the VBS approximation or comparison of theoretical predictions matched to parton shower) should 
